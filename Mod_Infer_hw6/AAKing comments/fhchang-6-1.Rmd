...
> You can use `scale_y_sqrt()` and `scale_y_log10` to transform the scales without changing the data.

> Check out `scale_x_date()`.
...
> It's not the normal distribution that is at issue here. Rather, consider the range of the data:
...
Here I plot the relationships between ~~$log\frac{N~t~}{N~t-1~}$ and $log(N~t-1~)$~~ **$\log\frac{N_t}{N_{t-1}}$** (left) as well as $log\frac{N~t~}{N~t-1~}$ and ${N~t-1~}$ (right) of parus and thrip population size. 
...
> Does the meaning of $r$ differ between the two models?
...
> Or could it be that the density dependence is seasonal?
...
> What models are the two tests comparing?
...
> I see some numbers, but no explanation of what you are calculating, or why.
...
> This is not how to interpret the AIC. Let's talk about this.

From the adjusted $R^2$, we see that the Gompertz model explain about `r round(summary(parus.lm.gomp)$adj.r.squared - summary(parus.lm.ricker)$adj.r.squared, 4)*100` % more variance than the Ricker model. 

> What is the difference between $R^2$ and $\bar{R}^2$?
...
> I don't follow what you are doing here.  Let's talk about this.
...
> Let's chat about this.  There are a number of questions that it is much easier to describe in person than in text.
