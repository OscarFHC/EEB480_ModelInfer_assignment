...
In terms of the amount of variance being explained, we can see that the $t-test$ performed on the slope estimation is identical to the $F-test$ performed on the model. This is because the influences of a explanatory variable in a model is the amount of variance explained by this explanatory variable. In this case, after include population size as explanatory variable, sum of squared error is being reduced from $`r var(parus.ex2$ratio)*(nrow(parus.ex2)-1)`$ (this is the total sum of square) to $`r anova(parus.lm.ricker)[2,2]`$. The amount of this reduction is the $R^2$ estimated in the linear model (`r summary(parus.lm.ricker)$r.squared`).  
...
Here I go on to calculate the standard error of the carrying capacity in the Gompertz model.

```{r}
B = summary(parus.lm.gomp)$coefficients[,1]
vcovB = vcov(parus.lm.gomp) # variance-covariance matrix of the linear model (Ricker model)
...
deltamethod(~ exp(x1/-x2), coef(parus.lm.gomp), vcov(parus.lm.gomp))
```
...
I therefore fit the linear model with interaction terms of population size and each month. By doing so, I can inspect whether the influences of population size on $log\frac{N_t} {N_{t-1}}$ are different in different month.
...
In this improved model, I found that the interaction term of month 4, 5, 8, 9, 10, 11 are significantly different from the January term. This indicates that the influences of population size on $log\frac{N_t} {N_{t-1}}$ in these months are different from the one in month 1.
...
From the 4 diagnostic plots, I see no clear structure of the residuals. The Breusch-Pagan test also suggests that the residual are homoscedasticity. 

## 12.b
```{r, thrip normality of residuals}
shapiro.test(residuals(thrip.season.gomp))
qqnorm(residuals(thrip.season.gomp))
qqline(residuals(thrip.season.gomp), col="red")
```

The Shapiro-Wilk test also shows nonsignificant result. This means the residuals are normally distributed. However, from the QQ plot, only point 30, 31, and 39 might be slightly deviated from the theoretical line. 
...
library(msm)
thrip.r.se = deltamethod(~ -log(-x2), coef(thrip.season.gomp), vcov(thrip.season.gomp))
thrip.K.se = deltamethod(~ exp(x1/-(x2)), coef(thrip.season.gomp), vcov(thrip.season.gomp))
```
